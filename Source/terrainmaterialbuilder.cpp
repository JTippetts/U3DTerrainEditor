...
void TerrainMaterialBuilder::SetDiffuseTextureNames(const std::vector<String> &filenames)
{
	if(!diffuseTextures_) return;
...
void TerrainMaterialBuilder::SetNormalTextureNames(const std::vector<String> &filenames)
{
	if(!normalTextures_) return;
	auto cache=GetSubsystem<ResourceCache>();
...
void TerrainMaterialBuilder::HandleEndFrame(StringHash eventType, VariantMap &eventData)
{
	...
	String matname="Materials/TerrainEdit8";
