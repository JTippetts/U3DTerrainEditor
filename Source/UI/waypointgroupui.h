...
class WaypointGroupUI : public Object
{
	...
	std::vector<ea::string> GetGroupNames();
	std::vector<Vector3> GetGroupKnots(const ea::string &group);
	WaypointGroup *GetGroup(const ea::string &name);
	...
	SharedPtr<WaypointGroup> CreateWaypointGroup(const ea::string &name);
