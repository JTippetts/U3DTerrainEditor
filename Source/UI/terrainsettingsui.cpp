...
void TerrainSettingsUI::HandleUpdate(StringHash eventType, VariantMap &eventData)
{
	...
	float near=nearfog->GetValue()/ nearfog->GetRange();
	float far=farfog->GetValue()/farfog->GetRange();
	
	float fogstart=near*1024.f;
	float fogend=fogstart+far*512.f;
...
{
	String curpath=element_->GetChildDynamicCast<LineEdit>("ProjectPath", true)->GetText();
...
void TerrainSettingsUI::HandlePickPathConfirm(StringHash eventType, VariantMap &eventData)
{
	if(eventData["OK"].GetBool())
	{
		...
		String fsfilename=fileSelector_->GetFileName();
