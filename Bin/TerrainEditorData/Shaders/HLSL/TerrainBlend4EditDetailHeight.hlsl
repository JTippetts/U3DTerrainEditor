    ...
    out float4 oColor : COLOR0)
{
    ...
    float4 weights = tex2D(sWeightMap0, iTexCoord).rgba;
	float mask=tex2D(sMask, iTexCoord).r;
    float sumWeights = weights.r + weights.g + weights.b + weights.a;
    weights /= sumWeights;
	
	float4 c1=blend((tex2D(sDetailMap1, iDetailTexCoord) + tex2D(sDetailMap1, iDetailTexCoord*-0.345))*0.5, weights.r,
		(tex2D(sDetailMap2, iDetailTexCoord)+tex2D(sDetailMap2, iDetailTexCoord*-0.345))*0.5, weights.g);
	float a1=weights.r+weights.g;
	float4 c2=blend((tex2D(sDetailMap3, iDetailTexCoord)+tex2D(sDetailMap3, iDetailTexCoord*-0.345))*0.5, weights.b,
		(tex2D(sDetailMap4, iDetailTexCoord)+tex2D(sDetailMap4, iDetailTexCoord*-0.345))*0.5, weights.a);
	float a2=weights.b+weights.a;
	
	float4 diffColor=cMatDiffColor * blend(c1,a1,c2,a2);
