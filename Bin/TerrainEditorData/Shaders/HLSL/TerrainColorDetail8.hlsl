...
sampler2D sColorMap : register(S4);
    ...
    out float4 oColor : COLOR0)
{
    ...
    weights0 /= sumWeights;
	weights1 /= sumWeights;
	float4 smallFrac=float4(frac(iDetailTexCoord.xy),iTexCoord.zw);
	float4 bigFrac=float4(frac(iDetailTexCoord.xy*-0.345), iTexCoord.zw);
	float4 diffColor=cMatDiffColor*tex2D(sColorMap, iTexCoord.xy).rgba;
    ...
    // Get normal
    #ifdef NORMALMAP
        float3x3 tbn = float3x3(iTangent.xyz, float3(smallFrac.zw, iTangent.w), iNormal);
        // We may be running low on instructions on Shader Model 2, so skip normalize if necessary
		float4 normalColor =(
			weights0.r * (tex2D(sDetailMap1, smallFrac*0.4961)) +
			weights0.g * (tex2D(sDetailMap1, smallFrac*0.4961+float2(0.502,0))) +
			weights0.b * (tex2D(sDetailMap1, smallFrac*0.4961+float2(0,0.502))) +
			weights0.a * (tex2D(sDetailMap1, smallFrac*0.4961+float2(0.502,0.502))) +
		
			weights1.r * (tex2D(sDetailMap2, smallFrac*0.4961)) +
			weights1.g * (tex2D(sDetailMap2, smallFrac*0.4961+float2(0.502,0))) +
			weights1.b * (tex2D(sDetailMap2, smallFrac*0.4961+float2(0,0.502))) +
			weights1.a * (tex2D(sDetailMap2, smallFrac*0.4961+float2(0.502,0.502)))
		);
