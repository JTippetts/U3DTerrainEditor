	...
	if input:GetMouseButtonDown(MOUSEB_LEFT) and ui:GetElementAt(mousepos.x, mousepos.y)==nil then
		...
		if self.mode==0 and input:GetQualifierDown(QUAL_CTRL) then
			...
			if ground~=nil then
				...
				elseif self.mode>=1 and self.mode<=8 then 
					ApplyBlendBrush8(terrain,hmap,blend1,blend2,mask,gx,gz,self.radius,self.max,self.power,self.hardness,self.mode,self.usemask,dt)
					blendtex1:SetData(blend1)
					blendtex2:SetData(blend2)
					if self.mode==1 then
						ApplySpeckleBrush(terrain,hmap,color,mask,gx,gz,self.radius,self.max,self.power,self.hardness,Color(0.75,0.75,0.75), Color(0.15,0.15,0.15), self.usemask,dt)
						colortex:SetData(color,false)
					elseif self.mode==2 then
						ApplySpeckleBrush(terrain,hmap,color,mask,gx,gz,self.radius,self.max,self.power,self.hardness,Color(0.1,0.15,0.1), Color(0.3,0.8,0.3), self.usemask,dt)
						colortex:SetData(color,false)
					elseif self.mode==3 then
						ApplySpeckleBrush(terrain,hmap,color,mask,gx,gz,self.radius,self.max,self.power,self.hardness,Color(0.15,0.08,0.1), Color(0.9,0.5,0.2), self.usemask,dt)
						colortex:SetData(color,false)
					elseif self.mode==4 then
						ApplySpeckleBrush(terrain,hmap,color,mask,gx,gz,self.radius,self.max,self.power,self.hardness,Color(0.5,0.2,0.3), Color(0.8,0.7,0.2), self.usemask,dt)
						colortex:SetData(color,false)
					elseif self.mode==5 then
						ApplySpeckleBrush(terrain,hmap,color,mask,gx,gz,self.radius,self.max,self.power,self.hardness,Color(0.05,0.05,0.06), Color(0.8,0.8,0.85), self.usemask,dt)
						colortex:SetData(color,false)
