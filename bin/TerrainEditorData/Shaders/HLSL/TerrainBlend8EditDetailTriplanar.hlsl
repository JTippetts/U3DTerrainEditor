    ...
    out float4 oColor : OUTCOLOR0)
{
	...
	float4 tex1=sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.zy, float2(0,0), lod1)*blending.x +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xy, float2(0,0), lod2)*blending.z +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xz, float2(0,0), lod3)*blending.y;
	float4 tex2=sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.zy, float2(1,0), lod1)*blending.x +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xy, float2(1,0), lod2)*blending.z +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xz, float2(1,0), lod3)*blending.y;
	float4 tex3=sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.zy, float2(2,0), lod1)*blending.x +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xy, float2(2,0), lod2)*blending.z +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xz, float2(2,0), lod3)*blending.y;
	float4 tex4=sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.zy, float2(3,0), lod1)*blending.x +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xy, float2(3,0), lod2)*blending.z +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xz, float2(3,0), lod3)*blending.y;
		
	float4 tex5=sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.zy, float2(0,1), lod1)*blending.x +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xy, float2(0,1), lod2)*blending.z +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xz, float2(0,1), lod3)*blending.y;
	float4 tex6=sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.zy, float2(1,1), lod1)*blending.x +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xy, float2(1,1), lod2)*blending.z +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xz, float2(1,1), lod3)*blending.y;
	float4 tex7=sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.zy, float2(2,1), lod1)*blending.x +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xy, float2(2,1), lod2)*blending.z +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xz, float2(2,1), lod3)*blending.y;
	float4 tex8=sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.zy, float2(3,1), lod1)*blending.x +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xy, float2(3,1), lod2)*blending.z +
		sampleTerrain(tDetailMap, sDetailMap, iDetailTexCoord.xz, float2(3,1), lod3)*blending.y;
    ...
    // Get normal
		...
		#else
		float3 bump1=DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.zy, float2(0,0), lod1))*blending.x +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xy, float2(0,0), lod2))*blending.z +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xz, float2(0,0), lod3))*blending.y;
		float3 bump2=DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.zy, float2(1,0), lod1))*blending.x +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xy, float2(1,0), lod2))*blending.z +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xz, float2(1,0), lod3))*blending.y;
		float3 bump3=DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.zy, float2(2,0), lod1))*blending.x +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xy, float2(2,0), lod2))*blending.z +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xz, float2(2,0), lod3))*blending.y;
		float3 bump4=DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.zy, float2(3,0), lod1))*blending.x +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xy, float2(3,0), lod2))*blending.z +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xz, float2(3,0), lod3))*blending.y;
			
		float3 bump5=DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.zy, float2(0,1), lod1))*blending.x +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xy, float2(0,1), lod2))*blending.z +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xz, float2(0,1), lod3))*blending.y;
		float3 bump6=DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.zy, float2(1,1), lod1))*blending.x +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xy, float2(1,1), lod2))*blending.z +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xz, float2(1,1), lod3))*blending.y;
		float3 bump7=DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.zy, float2(2,1), lod1))*blending.x +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xy, float2(2,1), lod2))*blending.z +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xz, float2(2,1), lod3))*blending.y;
		float3 bump8=DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.zy, float2(3,1), lod1))*blending.x +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xy, float2(3,1), lod2))*blending.z +
			DecodeNormal(sampleTerrain(tNormal, sNormal, iDetailTexCoord.xz, float2(3,1), lod3))*blending.y;
		#endif
